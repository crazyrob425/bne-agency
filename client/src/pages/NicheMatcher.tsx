...
function NicheArtifact({
  ...
  return (
    <motion.div
      ...
      className={highlight || active ? 'border-[#D4AF37]/60 ring-1 ring-[#D4AF37]/30 shadow-[0_0_24px_rgba(212,175,55,0.16)]' : 'border-white/5'}
    >
      <Link
        href={getNichePath(niche)}
        onClick={(event) => {
          event.preventDefault();
          onSelect(niche);
        }}
        className={`sapphire-glass lens-flare-hover group block p-5 diamond-cut text-left transition-all hover:-translate-y-0.5 hover:border-[#D4AF37]/40 ${highlight || active ? 'border-[#D4AF37]/60 ring-1 ring-[#D4AF37]/30 shadow-[0_0_24px_rgba(212,175,55,0.16)]' : 'border-white/5'}`}
        aria-label={`Open niche detail for ${niche.keyword}`}
      >
        ...
        <div className="mt-4 text-[10px] uppercase tracking-[0.35em] text-[#D4AF37]/80 font-black">
          Tap for deep dive
        </div>
        <span className="sr-only">{detail.intro}</span>
      </Link>
    </motion.div>
  );
}
...
function ProfileCase({
  ...
  return (
    <motion.div
      ...
      className={active ? 'border-[#D4AF37]/60 ring-1 ring-[#D4AF37]/30' : 'border-[#D4AF37]/30'}
    >
      <Link
        href={getNichePath(niche)}
        onClick={(event) => {
        ...
        className={`sapphire-glass bling-shine block p-8 diamond-cut text-left transition-all hover:border-[#D4AF37]/50 ${active ? 'border-[#D4AF37]/60 ring-1 ring-[#D4AF37]/30' : 'border-[#D4AF37]/30'}`}
        aria-label={`Open detailed niche card for ${niche.keyword}`}
      >
                         ...
                         <div className="space-y-16">
                            ...
                            <div className="grid gap-8 lg:grid-cols-3">
                               {quizResults?.matches.slice(0, 3).map((match, i) => (
                                 <ProfileCase key={match.niche.keyword} match={match} index={i} onSelect={openNicheCard} active={selectedNiche?.niche.keyword === match.niche.keyword} />
                               ))}
                            </div>
                               ...
                               <div className="pt-16 border-t border-white/5">
                                 ...
                                 <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                                   {quizResults.matches.slice(3, 11).map((match, i) => (
                                     <NicheArtifact key={match.niche.keyword} niche={match.niche} index={i + 3} onSelect={openNicheCard} active={selectedNiche?.niche.keyword === match.niche.keyword} />
                                   ))}
                                 </div>
                               </div>
                            )}
